      ...
      describe "各アイテムの情報表示・ボタンのテスト" do
        ...
        it "アイテム毎にID、親ユーザーID、親カテゴリーID、アイテム名、アイテムひらがな名、作成・更新日時が表示されること" do
          within("tr", text: item1.name) do
            # id、user_id、category_idカラムの列を指定して各idの値による他カラムの値との重複エラーが発生しないようにする
            expect(page).to have_selector("td:nth-child(1)", text: item1.id)
            expect(page).to have_selector("td:nth-child(2)", text: item1.user_id)
            expect(page).to have_selector("td:nth-child(3)", text: item1.category_id)
          ...
          within("tr", text: item2.name) do
            # id、user_id、category_idカラムの列を指定して各idの値による他カラムの値との重複エラーが発生しないようにする
            expect(page).to have_selector("td:nth-child(1)", text: item2.id)
            expect(page).to have_selector("td:nth-child(2)", text: item2.user_id)
            expect(page).to have_selector("td:nth-child(3)", text: item2.category_id)
