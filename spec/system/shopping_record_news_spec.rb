...
RSpec.describe "ShoppingRecordNews", type: :system do
  describe "ビューの要素" do
    describe "new" do
      context "サインインしている場合" do
        ...
        let!(:c1_preset_item_1) { create(:item, user: master_user, category: category_1) }
        let!(:c1_preset_item_2) { create(:item, user: master_user, category: category_1) }
        let!(:c1_preset_item_3) { create(:item, user: master_user, category: category_1) }
        let!(:c1_preset_item_4) { create(:item, user: master_user, category: category_1) }
        let!(:c1_preset_item_5) { create(:item, user: master_user, category: category_1) }
        let!(:c1_user_item) { create(:item, user: user, category: category_1) }
        let!(:c2_user_item) { create(:item, user: user, category: category_2) }
        let!(:c3_preset_items) { create_list(:item, 20, user: master_user, category: category_3) }
        let(:other_user) { create(:user) }
        let!(:other_user_item) { create(:item, user: other_user, category: category_1) }
        ...
        let!(:buy_today) do
          create(
            ...
            item_name: c1_preset_item_1.name, item_hiragana: c1_preset_item_1.hiragana,
        ...
        let!(:buy_yesterday) do
          create(
            ...
            item_name: c1_preset_item_2.name, item_hiragana: c1_preset_item_2.hiragana,
        ...
        let!(:buy_day_before_yesterday) do
          create(
            ...
            item_name: c1_preset_item_3.name, item_hiragana: c1_preset_item_3.hiragana,
        ...
        let!(:buy_six_days_ago) do
          create(
            ...
            item_name: c1_preset_item_4.name, item_hiragana: c1_preset_item_4.hiragana,
        ...
        let!(:buy_seven_days_ago) do
          create(
            ...
            item_name: c1_preset_item_5.name, item_hiragana: c1_preset_item_5.hiragana,
        ...
        it "カテゴリー項目に全てのカテゴリーが表示されること" do
          ...
          expect(page).to have_button(category_3.name)
        ...
        it "初期状態では表示カテゴリーの中で一番若いidのカテゴリーが選択されていること" do
          ...
          expect(page).to_not have_selector("button.active", text: category_3.name)
        ...
        it "選択しているカテゴリーのデフォルトアイテムを含むアイテムが表示されること", js: true do
          ...
          expect(page).to have_selector("button.active", text: category_1.name)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_1.name, visible: true)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_2.name, visible: true)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_3.name, visible: true)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_4.name, visible: true)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_5.name, visible: true)
          ...
          expect(c2_user_item.category).to eq category_2
        ...
        it "選択しているカテゴリーで別のユーザーが登録したアイテムが表示されないこと", js: true do
          ...
          expect(other_user_item.category).to eq category_1
        ...
        it "選択していないカテゴリーのアイテムが表示されないこと", js: true do
          ...
          # category_1のアイテムの非表示を確認
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_1.name, visible: false)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_2.name, visible: false)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_3.name, visible: false)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_4.name, visible: false)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_preset_item_5.name, visible: false)
        ...
        it "本日中に購入履歴があるアイテムに「今日購入してます」の表示があること" do
          expect(user.buys.purchased.where(item_name: c1_preset_item_1.name)).to exist

          expect(page).to have_selector("button.active", text: category_1.name)

          within("div.item-space", text: c1_preset_item_1.name) do
            expect(page).to have_content "今日購入してます"
          end
        end

        it "1日前に購入履歴があるアイテムに「昨日購入してます」の表示があること" do
          expect(user.buys.purchased.where(item_name: c1_preset_item_2.name)).to exist

          expect(page).to have_selector("button.active", text: category_1.name)

          within("div.item-space", text: c1_preset_item_2.name) do
            expect(page).to have_content "昨日購入してます"
          end
        end

        it "2日前に購入履歴があるアイテムに「2日前に購入」の表示があること" do
          expect(user.buys.purchased.where(item_name: c1_preset_item_3.name)).to exist

          expect(page).to have_selector("button.active", text: category_1.name)

          within("div.item-space", text: c1_preset_item_3.name) do
            expect(page).to have_content "2日前に購入"
          end
        end

        it "6日前に購入履歴があるアイテムに「6日前に購入」の表示があること" do
          expect(user.buys.purchased.where(item_name: c1_preset_item_4.name)).to exist

          expect(page).to have_selector("button.active", text: category_1.name)

          within("div.item-space", text: c1_preset_item_4.name) do
            expect(page).to have_content "6日前に購入"
          end
        end

        it "7日以上前に購入履歴があるアイテムに購入した年月日の表示があること" do
          expect(user.buys.purchased.where(item_name: c1_preset_item_5.name)).to exist

          expect(page).to have_selector("button.active", text: category_1.name)

          within("div.item-space", text: c1_preset_item_5.name) do
        ...
        it "同じアイテムの購入履歴が複数存在するときは最新の購入日がアイテムに表示されること" do
          ...
          expect(user.buys.purchased.where(item_name: c1_preset_item_1.name).count).to eq two_records
        ...
        it "表示されているアイテムをチェックできること" do
          ...
          within("div.item-space", text: c1_preset_item_1.name) do
            check c1_preset_item_1.name

            expect(find_field(c1_preset_item_1.name)).to be_checked
          end
        end
        ...
        it "アイテムをチェックするとアイテムのチェック数を示すポップアップが表示されること", js: true do
          ...
          within("div.item-space", text: c1_preset_item_2.name) do
            check c1_preset_item_2.name
            expect(find_field(c1_preset_item_2.name)).to be_checked
          end
        ...
        it "アイテムがチェックされていないときはアイテムのチェック数を示すポップアップが表示されないこと", js: true do
          ...
          # チェックを外してポップアップを消す
          within("div.item-space", text: c1_preset_item_1.name) do
            uncheck c1_preset_item_1.name
            expect(find_field(c1_preset_item_1.name)).to_not be_checked
          end
        ...
        it "リセットボタンをクリックするとアイテムのチェックが全て解除されること", js: true do
          ...
          # category_3のアイテムをチェックする
          click_button category_3.name
          expect(page).to have_selector("button.active", text: category_3.name)
      ...
      describe "ひらがなモードの設定により異なる箇所のテスト" do
        ...
        context "ひらがなモードOFF（デフォルト）の場合" do
          ...
          it "カテゴリー項目内でカテゴリー名（category.name）が表示されること" do
            expect(page).to have_selector("button", text: category_1.name)
          end

          it "カテゴリー項目内でひらがな（カテゴリー名）(category.hiragana)が表示されないこと" do
            expect(page).to_not have_selector("button", text: category_1.hiragana)
          end

          it "アイテム項目内でアイテム名(item.name)が表示されること" do
            expect(page).to have_selector("div.item-space", text: c1_preset_item_1.name)
          end

          it "アイテム項目内でひらがな（アイテム名）(item.hiragana)が表示されないこと" do
            expect(page).to_not have_selector("div.item-space", text: c1_preset_item_1.hiragana)
          end
        end
        ...
        context "ひらがなモードONの場合" do
          ...
          it "カテゴリー項目内でひらがな（カテゴリー名）(category.hiragana)が表示されること" do
            expect(page).to have_selector("button", text: category_1.hiragana)
          end

          it "カテゴリー項目内でカテゴリー名（category.name）が表示されないこと" do
            expect(page).to_not have_selector("button", text: category_1.name)
          end

          it "アイテム項目内でひらがな（アイテム名）(item.hiragana)が表示されること" do
            expect(page).to have_selector("div.item-space", text: c1_preset_item_1.hiragana)
          end

          it "アイテム項目内でアイテム名(item.name)が表示されないこと" do
            expect(page).to_not have_selector("div.item-space", text: c1_preset_item_1.name)
          end
        end
      end
    end
    ...
    describe "confirm", js: true do
      ...
      let!(:c1_preset_item_1) { create(:item, user: master_user, category: category_1) }
      let!(:c1_preset_item_2) { create(:item, user: master_user, category: category_1) }
      let!(:c2_preset_item) { create(:item, user: master_user, category: category_2) }
      # お買い物タイトルのデフォルトの入力値
      ...
      describe "ひらがなモードの設定により異なる箇所のテスト" do
        context "ひらがなモードOFF（デフォルト）の場合" do
          ...
          it "登録画面から投稿されたアイテムがアイテム名(item.name)で表示されること" do
            within ".confirm-window-text" do
              ...
              expect(page).to have_selector("li", text: c1_preset_item_1.name)
          ...
          it "登録画面から投稿されたアイテムがひらがな（アイテム名）(item.hiragana)で表示されないこと" do
            within ".confirm-window-text" do
              expect(page).to_not have_selector("li", text: c1_preset_item_1.hiragana)
          ...
          it "チェックされていないアイテム名は表示されないこと" do
            within ".confirm-window-text" do
              expect(page).to_not have_selector("li", text: c1_preset_item_2.name)
            end
          end
        end
        ...
        context "ひらがなモードONの場合" do
          ...
          it "登録画面から投稿されたアイテムがひらがな（アイテム名）(item.hiragana)で表示されること" do
            within ".confirm-window-text" do
              expect(page).to have_selector("h4", text: "買う予定のアイテム")
              expect(page).to have_selector("li", text: c1_preset_item_1.hiragana)
          ...
          it "登録画面から投稿されたアイテムがアイテム名(item.name)で表示されないこと" do
            within ".confirm-window-text" do
              expect(page).to_not have_selector("li", text: c1_preset_item_1.name)
          ...
          it "チェックされていないひらがな（アイテム名）は表示されないこと" do
            within ".confirm-window-text" do
              expect(page).to_not have_selector("li", text: c1_preset_item_2.hiragana)
            end
          end
        end
      end
  ...
  describe "お買い物登録のフロー" do
    describe "ユーザー区分で共通のフロー" do
      ...
      let!(:c1_item_1) { create(:item, user: master_user, category: category_1) }
      let!(:c1_item_2) { create(:item, user: master_user, category: category_1) }
      let!(:c2_item) { create(:item, user: master_user, category: category_2) }
      let(:default_title) { "#{Date.today.to_fs(:date_ja)}のお買い物" }
      ...
      context "正常系" do
        ...
        scenario "ユーザーがお買い物を登録する", js: true do
          ...
          expect(page).to have_selector("button.active", text: category_1.name)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_item_1.name, visible: true)
          expect(page).to have_selector(".item-name-space-shopping", text: c1_item_2.name, visible: true)

          check c1_item_1.name
          check c1_item_2.name
          ...
          within(".confirm-window", text: "買う予定のアイテム") do
            expect(page).to have_content c1_item_1.name
            expect(page).to have_content c1_item_2.name
          ...
          expect(shopping_record.buys.pluck(:item_name)).to include(c1_item_1.name, c1_item_2.name, c2_item.name)
        end
        ...
        scenario "フォームを入力して登録内容の確認画面から登録画面へ戻るとフォームの入力値が保持される", js: true do
          ...
          within(".confirm-window", text: "買う予定のアイテム") do
            expect(page).to have_content c1_item_1.name
            expect(page).to_not have_content c1_item_2.name
            expect(page).to have_content c2_item.name
          end
          ...
          expect(find_field(c1_item_1.name)).to be_checked
          expect(find_field(c1_item_2.name)).to_not be_checked
          click_button category_2.name
      ...
      context "異常系" do
        ...
        context "チェックできるアイテム数がチェック最大数を超える場合" do
          ...
          let!(:c3_items) { create_list(:item, 20, user: master_user, category: category_3) }
          ...
          let!(:unclosed_shopping_records_buys) do
            unclosed_shopping_records.each do |record|
              create(:buy, user: user, shopping_record: record,
                           item_name: c1_item_1.name, item_hiragana: c1_item_1.hiragana)
            end
          end
